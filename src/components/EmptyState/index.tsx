...
export function EmptyState() {
  const { loadFile, reloadWithEncoding, error, lowConfidence, MAX_SIZE } = useSubtitleLoader();
  ...
  return (
    <div className="mx-auto flex min-h-[calc(100vh-56px)] max-w-2xl flex-col items-center justify-center px-4">
      <div
        {...getRootProps()}
        className={`flex w-full cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed px-8 py-20 text-center transition ${
          isDragActive
      ...
      >
        ...
        <p className="mt-2 text-sm text-slate-400">支持 .srt · 单文件最大 5 MB · v1 仅处理字幕</p>
      </div>
