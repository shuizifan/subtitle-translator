...
- **多格式支持**：SRT、ASS/SSA、WebVTT、LRC，ASS 无损保留原有样式（VTT/LRC 统一导出为 SRT）
- **多服务商支持**：兼容 OpenAI、DeepSeek、Moonshot、智谱、Ollama 等所有 OpenAI 兼容接口
- **全片术语表**：先扫全片抽取人名/地名等专名并生成统一译名（可手工编辑），翻译时强制遵守，解决同一人名跨批次中英混用
- **译文体检**：导出前机械校验「原样返回原文 / 目标是中文却没有中文 / 长度比异常」，命中条目标黄并可一键重译
- **源字幕清理**：识别压制组问候语、字幕站水印、纯符号占位条、半角 `?` 冒充 ♪ 的歌词，可逐项开关、单条恢复
- **批量队列**：一次拖入多个文件或整个文件夹，逐个翻译后打包 ZIP 下载（保留原目录结构）
- **断点保护**：未完成的翻译自动存入 IndexedDB，刷新/关页后可「继续上次的翻译」
- **源语言自动判定**：按正文内容判断语种（容器/文件名里的语言标签常常不可信），顺带让双语文件名正确显示为「AI中英双语」
- **双语布局**：单轨（同条双行，默认）/ 双轨（同时间轴两条）/ 仅译文，语言顺序可调
...
当前支持格式：**SRT**、**ASS / SSA**、**WebVTT**、**LRC**（单文件 ≤ 20 MB）。
...
| 并发控制 | p-limit |
| 本地存档 | IndexedDB（未完成翻译）+ localStorage（设置） |
| 打包下载 | 内置极简 ZIP 写入器（`src/core/zip`，无额外依赖） |
| 测试 | Vitest |

架构：`上传 → Parser（SRT / ASS / VTT / LRC）→ 清理 + 语种判定 → SubtitleDocument（时间统一毫秒）→ 术语表 → Translator → 体检 → Serializer（双语 + 样式）→ 导出`
...
| 变量 | 默认 | 说明 |
|---|---|---|
| `ALLOW_PRIVATE_ENDPOINTS` | `1`（允许） | 是否允许转发到内网/回环地址。本机接 Ollama 需要保持允许；部署成公开站点时建议设为 `0`，避免转发路由被当作打内网的跳板 |
...
> 并把环境变量 `ALLOW_PRIVATE_ENDPOINTS` 设为 `0`。
